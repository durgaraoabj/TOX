...
<body>
	...
	<input type="hidden" name="subGroupInfoId" id="subGroupInfoId"
		value="${subGroupInfoId }" />
	<input type="hidden" name="crf" id="crf" value="${crf.id }" />
	<input type="hidden" name="sysDate" id="sysDate" value="${sysDate }" />
	<input type="hidden" name="dateVal" id="dateVal" value="${date}">
	<input type="hidden" name="crfCode" id="crfCode" value="${crf.crfCode}">
	<c:forEach items="${dates}" var="v">
		<%-- 		${v } ------  --%>
		<script>
			studyDates.push('${v}');
		</script>
	...
	<div class="box-header with-border">
		<c:if test="${type ne 'unscheduled'}">
			<div id="formulaMsg"
				style="color: red; text-align: center; font-size: medium; font-weight: bold;"></div>
			<table class="table table-bordered table-striped" style="width: 100%">
				<tr>
					<td>Study : ${sm.studyNo }</td>
					<td colspan="3">Observation Data Entry For :
						${crf.observationName}</td>
					<td><div>
							<c:if test="${type eq 'scheduled'}">
				...
				<tr>
					<td colspan="2">Accession No : ${animal.animalNo}</td>
					<td>Group : ${stdSubGroupObsCrf.subGroupInfo.group.groupName }</td>
					<td><c:if test="${sm.requiredSubGroup}">
						Sub Group : ${stdSubGroupObsCrf.subGroupInfo.name}
					</c:if></TD>
				</tr>
				<tr>
					<td colspan="2">Animal No : <c:if
							test="${type eq 'scheduled'}">
							${animal.permanentNo}
						</c:if>
					</td>
					<td colspan="2">Gender : <c:if test="${type eq 'scheduled'}">
							${animal.gender}
						</c:if>
					</td>
				</tr>
	...
	<div class="box-body">
		...
		<sf:form action="${studyCrfSave}" method="POST"
			...
			<input type="hidden" name="frequntlyMessage" id="frequntlyMessage"
				value="">
			<input type="hidden" name="duplicateData" id="duplicateData"
				value="${duplicateData }">
			<%-- 			<input type="hidden" name="subGroupInfoAllId" id="subGroupInfoAllId" value="${animal.id}" /> --%>
																				...
																				<c:if test="${secview eq 'yes'}">
																					<c:if test="${item.pattren eq ''}">
																						<c:choose>
																							...
																							<c:otherwise>
																								<input type="text" class="form-control" value="${item.values}"
																									name="${itemkey}" id="${itemkey}"
																									onchange="textOnChangeSec('${item.id}','${itemkey}', this.value, 'text')"
																									onmouseover="checkCaliculation('${item.id}','${itemkey}')"
																									onfocus="checkCaliculation('${item.id}','${itemkey}')" />
			...
			<div style="text-align: center">
				...
				<a href="#" class="btn btn-primary"
					onclick="redirctPageFunction('${subGroupInfoId }')">EXIT</a>
			</div>
		</sf:form>
	</div>
	...
	$.each(autocompleteFields, function(itemkey, collectionCodeName){
		if(collectionCodeName == "Animal Number"){
			$("#"+itemkey).autocomplete({
			    ...
			    $.ajax( {
			    	url: mainUrl+"/studyExecution/animalNumber/"+ request.term+"/Treatment",
...
function redirctPageFunction(subgId){
...
// 						alert(k);
				...
				if(ruleFlag){
					if(confirm("Crf Contains "+requiredFlag+" Fields Rules violated.<br/>Do You Want To Continue With Discrebency?")){
...
// 		}
	...
	function finalSubmit(){
		var type = '${type}';
		if(type == 'unscheduled'){
			$("#stdSubGroupObservationCrfsId").val($("#observationId").val());
			$("#animalId").val($("#animalIdPk").val());
			$("#animalId").val($("#animalIdPk").val());
			$('#studyCrfSave').submit();
		}else{
			  var groupInfo = "";
				...
				var sysDate1 = $("#sysDate").val();
				var dateVal = $('#dateVal').val();
				var deviation = true;
		        if(deviation){
//		 			alert("sysDate1 == value : " +sysDate1 + "=="+ dateVal +"  "+(sysDate == dateVal) )
				if(sysDate1 == dateVal)
					deviation= false;
				}	
				var submit = false;
							$('#deviationMessage').val(deviationMessage);
					submit = true;
					$('#deviationMessage').val("");
				if(submit){
					var duplicateData = $("#duplicateData").val(); 
					if(duplicateData == 'no'){
...
//		 				alert("OK");
						...
						var deviationMessage = prompt("Data already entered for Current Date,Would you like to enter data again, please confirm.");
						if(deviationMessage != null){
							if(deviationMessage.trim() != ''){
								$("#frequntlyMessage").val(deviationMessage);
								var typeVal = $('#type').val();
								var animalFlag = true;
								if(typeVal != "Scheduled")
								   animalFlag = animalNumberFunction('animalVal', 'animalValMsg');
...
<!-- <script src='/TOX/static/js/sariorius/usbDataReading.js'></script> -->
</html>