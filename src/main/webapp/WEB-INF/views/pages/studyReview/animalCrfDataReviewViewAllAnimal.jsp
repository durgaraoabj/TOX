...
<!-- 				</tr> -->
            ...
            <div class="card-body">
            	<div style="width: 100%; overflow: scroll">
            		...
            		<c:url value="/studyReview/approveObservationNew" var="saveObservationReview" />
		        	...
		        	<input type="hidden" name="groupid" id="groupid" value="${data.group.id}">
		        	<input type="hidden" name="subGroupId" id="subGroupId" value="${data.subGroup.id}">
		        	<input type="hidden" name="studyId" id="studyId" value="${sm.id}">
		        	<div id="amendmentMessage" style="color: red; text-align: center; font-weight: bold; font-size: medium;"></div>
		        	<table class="table table-bordered table-striped">
						<tr>
							<th>Count</th>
...
<%-- 									${st.count } --%>
									<c:if test="${data.allowDataEntry}">
										<c:choose>
											<c:when test="${ele.reviewed eq false}">
												<input type="checkbox" name="descElement" value="${ele.id}" id="${ele.id}"/>
											</c:when>
											<c:otherwise>
												<input type="checkbox" name="descElement" value="${ele.id}" id="${ele.id}" checked="checked" disabled="disabled"/>
											</c:otherwise>
										</c:choose>
...
<%-- 										<br/>${eleData.value.id}						 --%>
		...
		<c:url value="/studyReview/createDiscrepencyNew" var="createDiscrepency" />
	...
	function approveObservations(){
		if($("#descall").is(":checked")){
			...
			if(desc > 0){
				...
				var result = asynchronousAjaxCall(mainUrl+"/amandment/checkStudyAmandments/"+studyId);
				if(result != "undefined"){
					if(result != ""){
						$('#amendmentMessage').html(result);
					}else {
						if(result == ""){
							$('#amendmentMessage').html(result);
							$("#saveReview").submit();
