...
<body>
    <div class="box-body">
    <input type="hidden" id="smNoOfGroups" value="${sm.noOfGroups }"/>
    <input type="hidden" id="smSubjects" value="${sm.subjects }"/>
    <input type="hidden" id="smSstartDate" value="${sm.startDate }"/>
    
    <c:url value="/administration/updateStudy" var="studyCreateUrl"/>
    <form:form action="${studyCreateUrl}" method="post" modelAttribute="sm" id="studyCreateFormId" >
    	<form:hidden path="id"/>
    	<table class="table table-bordered table-striped">
    		<tr>
    			<th colspan="4">Study-${sm.noOfGroups }</th>
    		</tr>
    		<tr>
    			<td>Study Number</td>
    			<td>
    				<form:input path="studyNo" type="text" class="form-control input-sm" 
    				onblur="checkStudyNumber(this.value, this.id)" id="studyNo"  readonly="readonly"/>
    		...
    		<tr>
    			<td>No. Of Groups</td>
    			<td>
    				<input type="text" name="noOfGroups" id="noOfGroups"placeholder="count" class="form-control input-sm"
	...
	function groupInfo(value){
		...
		if(batch != '' && batch > 0){
			...
			for(var i = 1; i<= batch; i++){
				...
				+"<input type='radio' name='groupGender"+i+"' value='Buth' checked/>Both<br/>"
...
$("#noOfGroups").val($("#smNoOfGroups").val());
// groupInfo($("#noOfGroups").val());
$("#subjects").val($("#smSubjects").val());
// $("#startDate").val($("#smSstartDate").val());
