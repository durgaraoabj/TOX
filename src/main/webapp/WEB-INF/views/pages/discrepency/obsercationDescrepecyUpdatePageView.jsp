...
<body>
<br/>
	<c:url value="/discrepancy/observationdiscrepancyCloseOrUpdate" var="urlval" />		
	...
	<table class="table table-bordered table-striped">
		<tr>
			<th>Study No</th><td>${scd.study.studyNo }</td>
			<tH>Animal No</th><td>${scd.subGroupAnimalsInfo.animal.animalNo  }</td>
			
		</tr>
		<tr>
			<th>Observation</th><td>${scd.crf.observationName }</td>
			<th>Field</th><td>${scd.secElement.name }</td>
		<tr>
			<th>Status</th><td>${scd.status }</td>
			<th>Old Status</th><td>${scd.oldStatus }</td></tr>	
		<tr>
			<th>Value</th><td>${scd.secEleData.value  }</td>	
		...
		<tr>
			...
			<td>
				...
				<c:if test="${eleType eq 'date'}">
					<input type="text" name="newValue" id="newValue"  autocomplete="off">
				...
				<c:if test="${eleType eq 'dateAndTime'}">
				...
				<c:if test="${eleType eq 'time'}">
					<input class="form-control" type="text" name="newValue" id="newValue"  placeholder="Now" autocomplete="off">
					<script type="text/javascript">
				...
				<c:if test="${eleType eq 'selectTable'}">${htmlField}</c:if>
			</td>
