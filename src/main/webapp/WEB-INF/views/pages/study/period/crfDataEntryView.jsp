...
<body>
	<div class="box-header with-border">
		<table class="table table-bordered table-striped">
			<tr><td colspan="4">CRF Data Entry For the Observation : ${crf.}</td></tr>
					...
					<c:if test="${sec.containsGroup eq false}">
						...
						<table class="table table-bordered table-striped">
							<c:if test="${sec.hedding ne ''}">
								<tr><td colspan="${scol}"><b>${sec.hedding} </b></td></tr>
							</c:if>
							<c:if test="${sec.subHedding ne ''}">
								<tr><td colspan="${scol}"><b>${sec.subHedding}</b></td></tr>
							</c:if>
																...
																<c:if test="${item.totalDesc eq ''}">
																	...
																	<td><c:choose>
																			...
																			<c:when test="${item.type eq 'textArea'}">
																				...
																				<c:if test="${secview ne 'yes'}">
																					<textarea class="form-control" rows="3" cols="10" name="${itemkey}"
																							id="${itemkey}" disabled="disabled"></textarea>
																				</c:if>
																				...
																				<c:if test="${secview ne 'yes'}">
																					...
																					<c:if test="${item.display eq 'vertical'}">
																						<c:forEach items="${item.elementValues}"
																							var="itemval">
																							<input type="checkbox" name="${itemkey}"
																								id="${itemkey}" disabled="disabled" value="${itemval.value}"> ${itemval.value}<br>
																						</c:forEach>
																					</c:if>
																				</c:if>
																			...
																			<c:when test="${item.type eq 'date'}">
																				<c:if test="${secview eq 'yes'}">
																					<input type="text" class="form-control input-sm" value="" name="${itemkey}"
																						id="${itemkey}" autocomplete="off"
																				...
																				<c:if test="${secview ne 'yes'}">
																					<input type="text" class="form-control input-sm" value="" name="${itemkey}"
																						id="${itemkey}" autocomplete="off" disabled="disabled" />
																				</c:if>
...
<%-- 																			${item.type} --%>
					...
					rowsCurrentInfo['${sec.group.id}'] = ${gminrows};
						...
						<table class="table table-bordered table-striped"
							...
							<thead>
								<c:if test="${sec.hedding ne ''}">
									<tr><td colspan="${gcol}"><b>${sec.hedding}</b></td></tr>
								</c:if>
								<c:if test="${sec.subHedding ne ''}">
									<tr><td colspan="${gcol}"><b>${sec.subHedding}</b></td></tr>
								</c:if>
								<c:if test="${sec.group.hedding ne ''}">
									<tr><td colspan="${gcol}"><b>${sec.group.hedding}</b></td></tr>
								</c:if>
								<c:if test="${sec.group.subHedding ne ''}">
									<tr><td colspan="${gcol}"><b>${sec.group.subHedding}</b></td></tr>
								</c:if>
								<tr>
									<td colspan="${gcol}"><c:if test="${secview eq 'yes'}">
											...
											<input type="button" value="Add" id="addid" disabled="disabled">
											<input type="button" value="Remove" id="removeid" disabled="disabled">
										</c:if></td>
								</tr>
							</thead>
							<tbody>
															...
															<c:when test="${item.type eq 'radio'}">
																<c:if test="${item.display eq 'horizantal'}">
																	<c:forEach items="${item.elementValues}" var="itemval">
																		...
																		<c:if test="${secview ne 'yes'}">
																			<input type="radio" name="${itemkey}" id="${itemkey}"
																			value="${itemval.value}" disabled="disabled"> ${itemval.value}
																		</c:if>
																	</c:forEach>
																</c:if>
																<c:if test="${item.display eq 'vertical'}">
																	<c:forEach items="${item.elementValues}" var="itemval">
																		...
																		<c:if test="${secview ne 'yes'}">
																			<input type="radio" name="${itemkey}" id="${itemkey}"
																				value="${itemval.value}" disabled="disabled"> ${itemval.value}<br />
																		</c:if>
																	</c:forEach>
																</c:if>
																<font id="${itemkey}_msg" color="red"></font>
															...
															<c:when test="${item.type eq 'checkBox'}">
																<c:if test="${item.display eq 'horizantal'}">
																	<c:forEach items="${item.elementValues}" var="itemval">
																		...
																		<c:if test="${secview ne 'yes'}">
																			<input type="checkbox" name="${itemkey}"
																			id="${itemkey}" disabled="disabled"> ${itemval.value}
																		</c:if>
																	</c:forEach>
																</c:if>
																<c:if test="${item.display eq 'vertical'}">
																	<c:forEach items="${item.elementValues}" var="itemval">
																		...
																		<c:if test="${secview ne 'yes'}">
																			<input type="checkbox" name="${itemkey}" disabled="disabled"
...
<%-- 															${item.type} --%>
			...
			<div style="text-align: center">
				...
				<input type="checkbox" name="inReview" id="inReview"
					value="IN REVIEW">Sent to Review <input type="button"
					value="SAVE" id="formSubmitBtn" class="btn btn-primary">
				</c:if>
				<a
...
// 		alert(dbid + " : " + id + " : " + value + " : " + eletype);
		...
		if(value.trim() != ''){
			...
			if(dp) checkDateComparesion(dbid, id, value , eletype);
		}
...
//	 			var pattrenFlag  = pattrenValidation();
		        ...
		        if($("#inReview").prop("checked") == true)
		        	message = "Are you sure to save Crf Data and Sent to Review ?";
