...
<c:url value="/buildStdyCrf/crfRuleSave" var="crfRuleSave" />
		<sf:form action="${crfRuleSave}" method="POST"  id="crfSave" enctype="multipart/form-data">
			...
			<table id="tableId" class="table table-bordered table-striped">
	    		<thead>
		    		<tr>
		    			<th colspan="6">Other E-From Elements</th>
		    		</tr>
	    		</thead>
	    		<tbody>
		    		<tr id="fixed">
...
function secEleSelection(id, value){
	...
	var rowId = 0;
	function addRroele(){
		var result = asynchronousAjaxCall(mainUrl+"/buildStdyCrf/crfRuleElements/"+rowId);
		if(result != '' || result != 'undefined'){
			rowId ++;
...
<script type="text/javascript">
	...
	function groupEleSelection(id, value){
		if(value != -1){
			...
			var rows = nameArr[1];
			var rowsele = "Row No : <select name='groupEleIdRow' id='groupEleIdRow'><option value='1' selected='selected'>1</option> ";
...
function otherCrf(id, value, count){
	if(id == -1){
		...
		var result = asynchronousAjaxCall(mainUrl+"/buildStdyCrf/otherCrfSectionElements/"+value+"/"+count);
		if(result != '' || result != 'undefined'){
			$('#secEleIdTd'+count).html(result);
		}
		result = asynchronousAjaxCall(mainUrl+"/buildStdyCrf/otherCrfGroupElements/"+value+"/"+count);
		if(result != '' || result != 'undefined'){
