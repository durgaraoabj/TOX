...
<body>
		...
		<c:forEach items="${mp.value}" var="obj">
			<div id="${obj.id}_${mp.key}" style="display: none">
				<table>
					...
					<c:forEach items="${obj.stdSubGroupObservationCrfs.animals}" var="animal">
						<tr>
							<td><a
	...
	<c:forEach items="${unSchudeltreatmentMap}" var="accMap">
		<script>
			...
			map['end'] = '${accMap.value.end}';
			map['schduleType'] = '${accMap.value.schduleType}';
			unscheduledTreatmentjson['${accMap.key}'] = map;
		</script>
	</c:forEach>

	<c:url value="/studyReview/reviewAccessionForm" var="saveValues" />
	<form:form action="${saveValues}" method="post" id="valuesForm">
		<input type="hidden" name="studyAcclamatizationDatesId" id="studyAcclamatizationDatesId">
		<input type="hidden" name="crfId" id="crfId">
		<input type="hidden" name="type" id="type" value="scheduled">
		<input type="hidden" name="seletedGender" id="acclamigender"
			value="${study.gender}">
		<input type="hidden" name="studyId" id="studyId" value="${study.id}">
		<input type="hidden" name="dateString" id="dateString">
	</form:form>

	<c:url value="/studyReview/reviewObservationForm"
		var="treatementFromUrl" />
	<form:form action="${treatementFromUrl}" method="post"
		id="treatementForm">
		<input type="hidden" name="schduleType" id="schduleType">
		<input type="hidden" name="studyTreatmentDataDatesId" id="studyTreatmentDataDatesId">
		<input type="hidden" name="crfId" id="crfId">
...
<script type="text/javascript">
	...
	function gotoDataEntryFrom(dataId, dateString, groupId) {
		...
		if (groupId == 'Acclamatization') {
			...
			debugger;
			if (groupId == 'Treatment') 
				map = treatmentjson[dataId];
			else
				map = unscheduledTreatmentjson[dataId];
			activityType = "Treatment";
		}

		var navigation, crfId, studyId, action, divId, title, start, end, subGroupId, observationGener, primaryKey, 
		studyAcclamatizationDatesId,studyTreatmentDataDatesId, stdSubGroupObservationCrfsId, schduleType;
		...
		$.each(map, function(key, value) {
			...
			} else if (key == 'schduleType') {
				schduleType = value;
			} 
			
		});

		var startDay = new Date(dateString);
...
// 				&& currentDate.getTime() <= endDate.getTime()) {
			...
			if (activityType == 'Treatment') {
				debugger;
				if(schduleType == 'scheduled'){
					$("#studyTreatmentDataDatesId").val(studyTreatmentDataDatesId);
				}
				$("#schduleType").val(schduleType);
				$("#seletedGender").val(observationGener);
				$("#stdSubGroupObservationCrfsId").val(primaryKey);
				$("#treatementForm").submit()					

			} else {
				$("#studyAcclamatizationDatesId").val(studyAcclamatizationDatesId);
				$("#acclamigender").val(observationGener);
				$("#valuesForm").submit();
			}
// 		} else if (currentDate < startDate) {
// 			console.log('The current date is lessthan the start');
// 			alert("Date Review Not Possible Before "
// 					+ moment(startDate).format('YYYY-MM-DD') + ".")
// 		} else if (currentDate > endDate) {
// 			console.log('The current date is lessthan the start');
// 			alert("Date Review Not Possible After "
// 					+ moment(endDate).format('YYYY-MM-DD') + ".")
// 		} else {
// 			console
// 					.log('The current date is not between the start and end dates.');
// 			alert("Date Review Not Possible. Observation has out of Start "
