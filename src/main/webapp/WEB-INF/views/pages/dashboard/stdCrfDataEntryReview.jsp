...
var rowsLibInfo = {};
	...
	<div class="box-header with-border">
		<table id="example1" class="table table-bordered table-striped">
	    	<tr><td>CRF Data For the CRF : ${crf.title}</td><td>Study : ${sm.studyNo }</td><td>Phase : ${period.name }</td><td>Subject : ${vol.volId }</td><td>Bed No : ${vol.bedNo }</td></tr>
	    </table>
	</div>
	<div class="box-body">
			...
			<c:if test="${sec.active}">
				<c:if test="${sec.containsGroup eq false}">
					...
					<table class="table table-bordered table-striped">
						<c:if test="${sec.hedding ne ''}"><tr><td colspan="${scol}"><h5><b>${sec.hedding}</b></h5></td></tr></c:if>
...
<%-- 											<input type="text" value="${itemvalue}" name="${itemkey}" id="${itemkey}"/> --%>
		...
		<br/>
		    <div style="text-align:center">
		    	<a href='<c:url value="/globalStudyView/viewSubjectPeriodCrfs/${vol.id}/${period.id}" />' 
